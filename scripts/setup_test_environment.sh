...
install_chartmuseum() {
    if [ ! -f "testbin/chartmuseum" ]; then
        mkdir -p testbin/
        pushd testbin/
        curl -L https://get.helm.sh/chartmuseum-${CHARTMUSEUM_VERSION}-${PLATFORM}-amd64.tar.gz | tar xz ${PLATFORM}-amd64/chartmuseum
        mv ${PLATFORM}-amd64/chartmuseum ./chartmuseum
